...
export default function Profile() {
  ...
  const { location, detectLocation, updateLocation, searchLocationByCity, loading: loadingLocation } = useLocation();
  ...
  const languages = [
    { code: 'fr', name: t('French'), flag: 'https://flagcdn.com/w40/fr.png' },
    { code: 'ar', name: t('Arabic'), flag: 'https://flagcdn.com/w40/tn.png' },
    { code: 'en', name: t('English'), flag: 'https://flagcdn.com/w40/us.png' },
  ];
  ...
  const renderMain = () => (
    <div className="h-full pb-4" style={{ background: 'var(--bg-color)' }}>
      ...
      <div className="px-4 py-6 flex justify-between items-center border-b sticky top-0 z-10" style={{ background: 'var(--header-bg)', borderColor: 'var(--border-color)' }}>
        <h1 className="text-3xl font-black tracking-tight" style={{ color: 'var(--text-color)' }}>{t('Profile')}</h1>
        <button 
          onClick={() => navigate(-1)}
              ...
              <div className="p-6 text-center border-b" style={{ borderColor: 'var(--border-color)' }}>
                ...
                <div className="flex justify-center gap-4">
                  <button 
                    ...
                    className="w-12 h-12 rounded-full border flex items-center justify-center hover:scale-110 transition-transform active:scale-95 shadow-sm"
              ...
              <div className="flex items-center gap-2">
                {languages.map(l => (
                  <img loading="lazy" 
                    ...
                    src={l.flag} 
                    alt={l.name} 
                    className={`w-5 h-3.5 object-cover shadow-sm transition-all ${i18n.language === l.code ? 'opacity-100 scale-110' : 'opacity-40 grayscale'}`}
                    referrerPolicy="no-referrer"
                  />
                ))}
              </div>
            ...
            <div className="p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                >
                  ...
                  {loadingLocation ? t('Detecting...') : t('Auto-Detect')}
                </button>
              </div>
            </div>
            
            <div className="p-4 flex items-center justify-between">
              <span className="text-xs font-bold" style={{ color: 'var(--text-color)', opacity: 0.6 }}>
                {t('Timezone')}: {location.timezone || Intl.DateTimeFormat().resolvedOptions().timeZone}
              </span>
              <div className="flex gap-2">
                <button 
                  onClick={async () => {
                    ...
                    if (city) {
                      const success = await searchLocationByCity(city);
                      if (!success) {
                        alert(t('Could not find location. Please try another city.'));
                      }
                    }
                  }}
        ...
        <section>
          ...
          <div className="rounded-2xl border overflow-hidden shadow-sm divide-y" style={{ background: 'var(--header-bg)', borderColor: 'var(--border-color)' }}>
            ...
            <button 
              ...
              className="w-full p-4 flex items-center gap-4 hover:bg-[var(--card-bg)] transition-colors cursor-pointer"
            >
              <div className="w-8 h-8 rounded-lg flex items-center justify-center pointer-events-none" style={{ background: 'var(--trending-bg)' }}>
                <Trash2 className="w-5 h-5" style={{ color: 'var(--accent-color)' }} />
              </div>
              <span className="text-sm font-bold uppercase tracking-tight pointer-events-none" style={{ color: 'var(--text-color)' }}>{t('Clear cache')}</span>
            </button>
          </div>
        </section>
              ...
              <div className="flex flex-col items-center text-center space-y-4">
                ...
                <div className="flex flex-col w-full gap-3 mt-4">
                  <button 
                    ...
                    className="w-full py-4 rounded-xl font-black uppercase tracking-widest bg-[var(--accent-color)] text-white hover:opacity-90 transition-colors"
                  >
                    {t('Confirm')}
                  </button>
                  <button 
  ...
  const renderLanguage = () => (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-[var(--hover-bg)]0 backdrop-blur-sm" onClick={() => setView('main')}>
      <div className="w-full max-w-sm border rounded-2xl overflow-hidden shadow-2xl" style={{ background: 'var(--header-bg)', borderColor: 'var(--border-color)' }} onClick={(e) => e.stopPropagation()}>
